...
int main()
{
	...
	String source = s("foreign define println(x) var i = 0 repeat until i == 10 { println(i) i += 1 }");
	...
	while (true)
	{
		...
		//if (pq_inst_needs_arg(it.type))
		//{
		//	println("-> %-3d | %-20s %d", vm.ip, pq_inst_to_c_str(it.type), it.arg);
		//}
		//else
		//{
		//	println("-> %-3d | %-20s", vm.ip, pq_inst_to_c_str(it.type));
		//}
